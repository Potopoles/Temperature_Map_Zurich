...
for hash in hashes:
    ...
    for i,var in enumerate(model[hash]['varNames']):
        for order in model[hash]['orders'][i]:
            if var == 'T':
                ws_var_str = 'Tout'
                value = ws.loc[sel_dt_str][ws_var_str]**order
            if var == 'wind':
                ws_var_str = 'wind'
                value = ws.loc[sel_dt_str][ws_var_str]
                if value < 1E-10:
                    value = 0.00001
    ...
    model[hash]['pred_absT'] = predict + ws.loc[sel_dt_str]['Tout']
...
##zlim0 = ws.loc[sel_dt_str]['Tout'] - 5
##zlim1 = ws.loc[sel_dt_str]['Tout'] + 2
#
#
## color normalization
